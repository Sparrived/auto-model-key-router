...
7. **概览 / 用量统计 / 工作空间 / 服务日志**：概览看实时（RPM/TPM、脉搏、趋势、热力图、请求流——逐条给出模型路由、请求来源（调用方、工作空间与来源 IP）、Token 与结果），用量统计看历史（累计、按天、日内时段、维度明细——按模型、调用方、**上游 Key**（供应商 + Key 名）与**访问密钥**各拆一张表，窗口可到 1 年或全部），工作空间按空间拆用量并用桑基图给出请求流向（工作空间 → 请求模型 → 实际模型 → 供应商 → 上游 Key → 上游模型），服务日志单独成页用于排查。
