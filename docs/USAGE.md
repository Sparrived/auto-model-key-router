...
- `models`：模型列表；每个模型至少有一个 `targets[]`，通过 `{provider, key, upstream_model}` 绑定一个供应商 Key。可选 `aliases`（额外的可调用名，与模型 ID 一起出现在 `/v1/models`）；`upstream_model` 是发给上游的名字，不可调用（见 [对外名称与上游名称](#对外名称与上游名称)）。
...
| 模型路由 | 管理模型别名（对外名称）、路由模式，把供应商 Key 绑定/解绑到模型、修改每条目标的上游模型名、把目标移到其它路由 |
...
AMKR 会在转发给上游前把请求体里的模型名改写为选中 target 的 `upstream_model`（留空时即模型 ID），并把 `Authorization` 替换成选中的上游 `api_key`。

### 对外名称与上游名称

一个模型有两层名字，AMKR 把它们分开：

- **对外名称**：模型 ID 与 `aliases`。它们就是全部的可调用名，会出现在 `/v1/models` 与 `/health` 里，客户端请求体里的 `model` 写的就是其中之一。
- **上游名称**：每条 target 的 `upstream_model`，即"发给这个上游的名字"。它**不可调用**，只在转发前替换请求体里的 `model`；留空表示与模型 ID 相同。

同一模型在不同上游经常叫不同名字（例如对外统一叫 `deepseek-v4.1-flash`，某家上游只认 `deepseek-flash`），这时把它们都收进同一条路由的 `targets` 即可：外部只看到 `deepseek-v4.1-flash`，轮询到哪个 Key 就用它自己的上游名。

```json
{
  "models": {
    "deepseek-v4.1-flash": {
      "aliases": ["fast-deepseek"],
      "targets": [
        {
          "provider": "vendor-a",
          "key": "main",
...
上例中 `deepseek-v4.1-flash` 与 `fast-deepseek` 可调用且会出现在 `/v1/models`；`deepseek-flash` 既调不动也不会被列出——它只是发给 vendor-a 的名字。

规则：

- 想让某个上游名也能被直接调用，就把它写进 `aliases`（或干脆把路由改名叫那个名字）。
- 模型 ID 与所有 `aliases` 在整份配置里必须全局唯一，写配置时会校验；解析时按 `models` 的顺序取第一个匹配。
- 一条路由下没有 target 就不该存在：解绑最后一个 Key，或在 WebUI 里把目标删空并保存，这条路由会被自动删除，`unified_model` 与任务里对它的引用会一并清理。
- 已经没有「隐藏别名」这个概念：旧配置里遗留的 `hidden_aliases` / `auto_hidden_aliases` 会被直接忽略（不报错），需要保留的名字请挪进 `aliases`；管理 API 的请求里带上这两个字段会返回 `422`。
...
| 字段 | 说明 |
| --- | --- |
| 任务名 | 客户端要传的 `model`，如 `TASK_000001`；不能与模型 ID、别名或 `unified-model` 撞名 |
...
- 任务名是**虚拟模型名**，不会出现在 `/v1/models` 里。这与模型别名不同：别名是一个模型的名字，任务名则是一整组路由与参数的别名；如果需要客户端直接调用的名字能在 `/v1/models` 里看到，请给模型加一个别名。
...
- 工作空间**只隔离任务**。模型 ID、别名与 `unified-model` 仍然全局唯一，任务名也不能与它们撞名 —— 否则路由语义会取决于查表顺序，这是全局唯一的判断。
...
流带越粗表示该段承载的请求越多，因此「哪个团队的任务、打到了哪个模型的哪家供应商的哪把 Key」一眼可读。**上游 Key 这一层**尤其必要：同一家供应商可以配多把 Key，只画到供应商会让两把 Key 的流量并成一条。宽度可以在**请求数**与 **Token** 之间切换：前者看调用次数，后者看实际消耗（长上下文的空间在 Token 视图下会明显更粗）。时间窗口与「用量统计」页共用同一套（含「全部历史」）。
...
| 「用量统计」页 | 按模型 / 调用方 / **上游 Key** / **访问密钥** / 模型-上游 Key 维度的历史用量明细表（含 Token 与缓存率），成本明细见「成本」页 |
